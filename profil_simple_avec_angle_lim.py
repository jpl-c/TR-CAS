...
def rotation(theta, H):
    ...
    ### On travaille dans la base tournée de theta

    T = np.array([0, H])
    psi_max = 0.7 
    i_max = 400

    sigma = 2*H*np.tan(psi_max)/5
    mu  = 0
    f = lambda t : 1/(sigma* np.sqrt(2*np.pi)) * np.exp(- (t-mu)**2/(2*sigma**2))
    ...
    for i in range(-i_max, i_max):
        ...
        ### Calcul de B, le pt d'intersection entre le rayon et le plan virtuel de la gaussienne
        B = np.array([H*np.tan(psi), 0])

        ### Calcul de D, le pt d'intersection entre le rayon et le plan réel de dépôt de la poudre
        d = 1/(1-np.tan(theta)*np.tan(psi))
        D = np.array([1, -np.tan(theta)])*H*np.tan(psi)*d

        ### Calcul des coordonnées de A, le pt d'intersection du rayon avec la gaussienne
        func_a = lambda a : f(a*H*np.tan(psi)) + H*(a-1)
        a = fsolve(func_a, 0)[0] #on ne garde que 3 chiffres après la virgule
        xa = a*H*np.tan(psi)
        ya = f(xa)
        A = np.array([xa, ya])

        ### Calcul de C, le point de la courbe réelle de dépôt de matière
        c = d + a - 1
        xc = c*H*np.tan(psi)
        yc = H*(1-c)
        C = np.array([xc, yc])
    ...
    ### Calcul des coordonnés de C dans la base initiale
        turned_profile_x.append(np.dot(Rot_mat(theta), C)[0])
        turned_profile_y.append(np.dot(Rot_mat(theta), C)[1])


    ### Calcul des points de la gaussienne tournée dans le repère initial
    x_gauss = np.linspace(-5, 5, 500)
    y_gauss = f(np.linspace(-5, 5, 500))
    x_gauss_r = np.empty_like(x_gauss)
    y_gauss_r = np.empty_like(y_gauss)

    for i in range(len(x_gauss)):
        X = np.array([x_gauss[i], y_gauss[i]])
        turned_X = np.dot(Rot_mat(theta), X)
        x_gauss_r[i] = turned_X[0]
        y_gauss_r[i] = turned_X[1]
    
    
    return np.array([turned_profile_x, turned_profile_y])
